...
import { MegolmOutbound, MegolmInbound } from "../crypto/pkg-node/atmin_crypto.js";

describe("Megolm", () => {
  describe("basic encrypt/decrypt", () => {
    it("roundtrips a message", () => {
      const sender = new MegolmOutbound();
      const receiver = MegolmInbound.from_session_key(sender.session_key());
  ...
  describe("multi-device (per-device sessions)", () => {
    ...
    it("Bob decrypts from both of Alice's devices", () => {
      ...
      const bobFromLaptop = MegolmInbound.from_session_key(laptop.session_key());
      const bobFromPhone = MegolmInbound.from_session_key(phone.session_key());

      expect(bobFromLaptop.decrypt(laptop.encrypt("from laptop"))).toBe("from laptop");
      expect(bobFromPhone.decrypt(phone.encrypt("from phone"))).toBe("from phone");
