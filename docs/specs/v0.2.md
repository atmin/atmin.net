...
The wire format migrates **once**; implementation ships in stages
(single-image preview + optimized send + strip first, multi-select album UI
later with no further wire change). Related tasks:
[lazy-load-media](../../tasks/lazy-load-media.md),
[archive-ingest-cache](../../tasks/archive-ingest-cache.md), and a Phase-1
task to follow.
