...
4. **[storage-indicator](storage-indicator.md)** — `GET /v1/storage/usage` endpoint backed by the existing quota cache, surfaced as a "X MB / 1 GB" line in settings with a warning at 90%. Straightforward now that media upload has landed and the quota infrastructure is in place.
